...
class AbstractGraph {
   ...
   */
  virtual AbstractGraph* add(const V& vertex) = 0;
  /**
   * @params
   *  vertex - vertex should be removed to graph
   * @return "this" for chaining, could be replaced by void
   */
  virtual AbstractGraph* remove(const V& vertex) = 0;
  /**
   * @params
   ...
   * @return "this" for chaining, could be replaced by void
   */
  virtual AbstractGraph* connect(const V& vertex1, const V& vertex2, const E& edge) = 0;
  /**
   * @params
   *  vertex1, vertex2 - graph verticies beetwen edge should be removed
   * @return "this" for chaining, could be replaced by void
   */
  virtual AbstractGraph* disconnect(const V& vertex1, const V& vertex2) = 0;
  /**
   * @params
   *  vertex - vertext to check
   * @return true if graph contains vertex
   */
  virtual bool contains(const V& vertex) = 0;
  /** 
